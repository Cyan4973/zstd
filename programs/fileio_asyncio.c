...
static void AIO_IOPool_createThreadPool(IOPoolCtx_t* ctx, const FIO_prefs_t* prefs) {
    ...
    if(prefs->asyncIO) {
        ...
        ctx->threadPool = FIO_TPool_create(1, MAX_IO_JOBS - 2);
...
static void AIO_IOPool_init(IOPoolCtx_t* ctx, const FIO_prefs_t* prefs, void (*poolFunction)(void*), size_t bufferSize) {
    int i;
...
static void AIO_IOPool_enqueueJob(IOJob_t* job) {
    IOPoolCtx_t* const ctx = (IOPoolCtx_t *)job->ctx;
    if(AIO_IOPool_threadPoolActive(ctx))
        FIO_TPool_submit(ctx->threadPool, ctx->poolFunction, job);
    else
